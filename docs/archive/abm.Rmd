...
# extract the numeric part of the file names and order them according to this
file_paths <- file_paths[order(as.numeric(gsub("\\D", "", basename(file_paths))))]

#length(file_paths) == length(nets) # not for all starting networks a simulation was performed...

# extract simulation number (corresponding to starting network id)
number <- as.numeric(gsub(".*_(\\d+)\\.rds", "\\1", file_paths))

# find the networks for which we don't have a simulation df;
missing_number <- setdiff(c(1:length(nets)), number)
...
file_paths <- list.files("./sims/", full.names = TRUE)
file_paths <- file_paths[order(as.numeric(gsub("\\D", "", basename(file_paths))))]
#length(file_paths) == length(nets) # Check

# use lapply to read all .rds files and rbindlist to combine them
system.time(all_data <- rbindlist(lapply(file_paths, function(file) {
  num_part <- as.numeric(gsub("\\D", "", file)) 
  data <- tryCatch(readRDS(file), error = function(e) NULL)
  if (!is.null(data)) {
    data <- data.table(data)
    data[, network := num_part]  #also add the 'network number'
  }
  return(data)
}), fill = TRUE))

# subset all networks in our df for which also have a simulation
df2 <- df[all_data$network,]
sims <- cbind(df2, all_data)
...
```{r}
data <- fload("./data/processed/20250217sim_dataframe.Rda")
names(data)[10] <- "i"
...
#create_heatmap(unique(data$p_t)[2]) + labs(title=NULL) + theme(legend.position = "right")
#ggsave("./figures/neg_cascade_10.png", height=4.5, width = 6.5)

#(heatmap1 + heatmap3) +
#  plot_layout(guides = "collect") & theme(legend.position = "bottom")
#ggsave("./figures/neg_cascade_5_20.png", height=4.5, width=7.5)
```
...
# let's get networks with max-degree <35
ind2 <- ind[which(sapply(nets[ind], function(net) max(degree(net$network)) < 35))]

for (i in unique(ind2)) { 
  graph_plot <- ggplotify::as.grob(~fplot_graph(nets[[i]]$network))
  ...
  title_grob <- textGrob(paste("Network Index:", i), gp = gpar(fontsize = 12, fontface = "bold"))

  grid.arrange(
    arrangeGrob(title_grob, graph_plot, ncol = 1, heights = c(0.1, 1)), 
    abm_plot,  
    ncol = 2, widths = c(1, 1)  )
}
```
...
```{r}
# a function to generate a gif that visualizes the spread of the norm
fnetworkgif <- function(network, decision_history, rounds, output_dir = "./figures") {
...
```{r, eval=FALSE}
# visualize the emergence of an unpopular norm for all candidates for our epxerimental condition:
for (i in ind2) {
  network <- nets[[i]]$network
  decisions <- fabm(
  network = network,
  rounds = 20,
  params = list(s = 15, e = 10, w = 40, z = 50, lambda1 = 4.3, lambda2 = 1.8),
  plot = TRUE, histories=TRUE)$decision_history
  
  # generate the gif for the current network
  gif_filename <- paste0("./figures/animation_network_", i, ".gif")
  gif_path <- fnetworkgif(network, decisions, rounds = 20, output_dir = "./figures")
...
gif_files <- list.files("./figures/", pattern = "animation_network_.*\\.gif", full.names = TRUE)
knitr::include_graphics(c(gif_files))
``` 
