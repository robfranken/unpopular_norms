...
## 1. scale-free network with central minorities
...
```{r}
test <- run_one_seed(2, k_min = 2, k_max = 30, alpha = 2.4, rho = 0.4, r = -0.1, return_network = TRUE)
seed = seed + 1
...
```{r, eval = FALSE, echo=TRUE, fig.show='hold', fig.keep='all', message=FALSE, fig.height=5}
run_one_sw <- function(
  i,
  base_seed = 1253281,
...
```{r, eval =FALSE}
test <- run_one_sw(i=15, model = "caveman", clique_size = 5, pmin = .15)
seed = seed + 1
